        ...
        <div class="row justify-content-between align-items-center">
            <div class="col-12 col-lg-12">
                <div class="card border-left-warning shadow border-2">
                    <div class="card-body">
                        <h2 class="text-medium text-center fw-bold mb-3">{{ __('Daily Customer Order') }}</h2>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @php
                                                    ...
                                                    if ($statuses->every(fn($status) => $status == 2)) {
                                                        $orderStatus = __('Success');
                                                    } elseif ($statuses->every(fn($status) => $status == 1)) {
                                                        $orderStatus = __('Pending');
                                                    } elseif ($statuses->every(fn($status) => $status == 3)) {
                                                        $orderStatus = __('Rejected');
                                                    } else {
                                                        $orderStatus = __('Partial Processing');
                                                    }
                                                @endphp
                                                <span
                                            ...
                                            <td>
                                                <a href="{{ route('order.viewOrder', $orderCode) }}"
                                                    class="btn btn-primary w-100">{{ __('View') }}</a>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
