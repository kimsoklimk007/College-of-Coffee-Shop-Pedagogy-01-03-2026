...
<div class="container mt-3">
        <h2 class="fw-bold text-center">{{ __('Asset Lists') }}</h2>
         <a href="{{ route('assets.create') }}" class="btn btn-primary mb-3">{{ __('Add Asset') }}</a>
         @if(session('success'))
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <form action="{{ route('assets.destroy', $asset->id) }}" method="POST" style="display:inline-block;">
                                @csrf
                                @method('DELETE')
                                <button class="btn btn-outline-danger rounded-pill btn-sm" onclick="return confirm('{{ __('Are you sure to delete this record?') }}')">
